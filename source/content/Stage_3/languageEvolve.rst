...
系统本身随着scale up 复杂性就会增加，最先出现的那就是变量的使用，下一个要出现的那就是分支出现。 各种情况最终都可以用分支与跳转来实现。 

但是 if else多了就会出现排列爆炸的问题，特别是多个独立条件出现的情况，if/else就是一个nightmare. 再复杂一些那就是if/else + dependencies的出现。

并且需求也会正交的如何把他们放在一块。一种就是function 执行水平的 hook-up,最简单的enter,exit,并且在此之后类似于python context的机制。 

再进一步使用 yield机制把中断机制也引入了执行各种上层的应用之中，中断机制是DLL的用时加载，延迟加载实现的基本原理，由于python的yeild机制也就需要原函数在调用一次，就简单实现状态方便的实现了状态机制，要比C语言中函数中static funciton variable,以及goto的要安全的多。

如果你在现有API的代码层面添加一层新的应用时，那就是元编程的用途。例如JIT + 元编程。再进一步灵活利用scheme这样的语言，可以的variable,if/else,while本身进行更加灵活的定制。 
